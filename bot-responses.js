    ...
    async function response(msg, ...sender_psid) {
      ...
      // if the message is set reminder then return the courses to choose from
        ...
        const getResponse = async (courses) => {
          ...
          let filteredCourses = await Promise.all(filteredCourses.map(async fCourse => {
...
async function response(msg, ...sender_psid) {
  ...
  // if the message is set reminder then return the courses to choose from
    ...
    const getResponse = async (courses, classroom) => {

      let passedString = '  SELECT A COURSE: \n'


      let filteredCourses = await Promise.all(courses.map(async fCourse => {
    ...
    // if the user isn't in the cache
      ...
      return await getResponse(filteredCourses, classroom);
    }
